...
$tabs['additional_information']['priority'] = __( 1 );
return $tabs;
}
...
function truemisha_new_tab_content() {
	...
	echo do_shortcode('[product_category category="dop-oborudovanie" columns="2" number="4" ]'); 
 
}

add_action( 'woocommerce_after_single_product_summary', 'show_info_after_product', 11 );

function show_info_after_product () {
	echo '<h2 style="font-size: 26px">Услуги: </h2>';
	the_field('слайдер_услуг_в_товаре');
}
...
/**woocommerce_template_single_meta - удаление таксономий из карточки под ценой */
...
if (empty($_product->get_price())) return __('
<span><a class="popup  popup_ap" href="">Узнать цену</a></span>
');
return $price;
}
...
add_action( 'woocommerce_after_single_product_summary', 'woocommerce_output_related_products', 13);
...
function popular_model () {
	echo '<h2 class="heading-28">Популярные модели</h2>';
	echo do_shortcode('[product_category category="popular-products" columns="4" number="4" ]');
}
